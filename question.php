...
// Javoblarni olish (misol uchun, buni faqat testlarni tekshirish uchun ishlatish mumkin)
$correctAnswers = [];
foreach ($questions as $question) {
    $correctAnswers[$question['id']] = $question['correct_answer']; // to'g'ri javobni olish
}
...
<body class="bg-white dark:bg-black text-black dark:text-white min-h-screen py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
    ...
    <div class="max-w-3xl mx-auto space-y-8">
        ...
        <?php foreach ($questions as $index => $question): ?>
            <div class="bg-gray-100 dark:bg-gray-900 bg-opacity-50 backdrop-filter backdrop-blur-lg rounded-xl shadow-lg p-6 space-y-4">
                <h3 class="text-xl font-bold text-black dark:text-white"><?= $index + 1; ?>. <?= $question['title']; ?></h3>
                <div class="space-y-2">
                    <?php foreach (['a', 'b', 'c', 'd'] as $variant): ?>
                        <label class="flex items-center space-x-3 p-2 rounded-md hover:bg-gray-200 dark:hover:bg-gray-800 transition duration-150 ease-in-out"
                            ...
                            <span class="text-gray-700 dark:text-gray-300"><?= $question[$variant . '_variant']; ?></span>
                        </label>
                    <?php endforeach; ?>
                </div>
    ...
    <script>
        ...
        let correctAnswers = <?= json_encode($correctAnswers); ?>;
        let userAnswers = {};
